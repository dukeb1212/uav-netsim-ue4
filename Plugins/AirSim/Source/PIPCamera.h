...
public:
    ...
    void SetPacketLossParam(const std::string& param_name, float value);
    ...
    UTextureRenderTarget2D* getRenderTarget(int32 type, bool if_active);
    ...
    void SetDesiredCaptureFPS(float NewFPS);

    UFUNCTION(BlueprintCallable, Category = "Capture Settings")
    float GetFPS() const { return FPS; }

    UFUNCTION(BlueprintCallable, Category = "Capture Settings")
    void SetFlowId(int32 NewFlowId) { FlowId = NewFlowId; }
...
private: //members
    ...
    UPROPERTY()
    UMaterialParameterCollection* PacketLossParamCollection;
    UPROPERTY()
    UMaterialParameterCollectionInstance* PacketLossParamInstance;
	UPROPERTY()
	TArray<UMaterialInstanceDynamic*> PacketLossMaterials;
	UPROPERTY()
	UMaterial* PacketLossMaterialStatic;
...
private: //methods
    ...
    void setPacketLossEffectMaterial(int image_type, UObject* outer, FPostProcessSettings& obj);
